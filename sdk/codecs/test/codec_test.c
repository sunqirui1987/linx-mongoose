...
// 测试编解码器工厂
int test_codec_factory(void) {
    printf("Testing codec factory...\n");
    
    // 测试获取支持的编解码器数量
    int count = codec_factory_get_supported_count();
    printf("Supported codec count: %d\n", count);
    assert(count > 0);
    
    // 测试获取支持的编解码器类型
    codec_type_t* types = codec_factory_get_supported_types();
    assert(types != NULL);
    
    for (int i = 0; i < count; i++) {
        const char* name = codec_factory_get_name(types[i]);
        printf("Supported codec %d: %s\n", i, name);
        assert(name != NULL);
    }
    ...
    const char* codec_name = codec->vtable->get_codec_name(codec);
    printf("Created codec: %s\n", codec_name);
    assert(strcmp(codec_name, "Opus") == 0);
    
    // 销毁编解码器
    codec_factory_destroy(codec);
    
    printf("Codec factory test passed!\n\n");
    return 0;
}
...
int test_opus_codec_basic(void) {
    ...
    // 初始化解码器
    result = codec->vtable->init_decoder(codec, &format);
    assert(result == CODEC_SUCCESS);
    assert(codec->decoder_initialized == true);
    
    // 测试获取参数
    int frame_size = codec->vtable->get_input_frame_size(codec);
    printf("Input frame size: %d samples\n", frame_size);
    assert(frame_size == FRAME_SIZE);
    
    int max_output_size = codec->vtable->get_max_output_size(codec);
...
int test_opus_codec_encode_decode(void) {
    ...
    for (int frame = 0; frame < NUM_FRAMES; frame++) {
        ...
        // 编码
        size_t encoded_size;
        result = codec->vtable->encode(codec, input_buffer, FRAME_SIZE * CHANNELS,
        ...
        // 解码
        size_t decoded_size;
        result = codec->vtable->decode(codec, encoded_buffer, encoded_size,
...
int test_error_handling(void) {
    ...
    codec_error_t result = codec->vtable->encode(codec, input, FRAME_SIZE,
                                                output, MAX_PACKET_SIZE, &encoded_size);
    assert(result == CODEC_INITIALIZATION_FAILED);
    
    // 测试无效参数
    result = codec->vtable->encode(NULL, input, FRAME_SIZE, output, MAX_PACKET_SIZE, &encoded_size);
...
int main(void) {
    ...
    // 运行所有测试
    if (test_codec_factory() != 0) return 1;
